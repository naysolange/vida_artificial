...
float actualizarCelula( float actual, float n, float ne, float e, float se, float s, 
  ...
  float proximo = int(random(3)) == 0 ? promedio(actual, n, ne, e, se, s, so, o, no) : mediana(actual, n, ne, e, se, s, so, o, no); 
...
private float promedio(float actual, float n, float ne, float e, float se, float s, 
  float so, float o, float no) {

  return (actual + n + ne + e + se + s + so + o + no)/9.0;
